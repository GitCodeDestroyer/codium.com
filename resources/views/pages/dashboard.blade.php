...
@section('styles')
    <style>@keyframes opacity {0%{opacity:1}50%{opacity:0}100%{opacity:1}}.nav .menu a:nth-of-type(1) {border-bottom: 2px solid #E9908A}</style>
...
@section('content')
    ...
    <div class="my-course" id="{{ $recent }}">
        <div class="decor">
            <img class="wow fadeIn" data-wow-duration="2.5s" src="{{ asset('img/icons/'.$recent.'-plain-white.svg') }}" alt="">
            <div class="line">
            ...
            <div class="type fadeInDown" data-wow-duration="2.5s" style="margin-top: {{ $margin.'px' }}"> {{ $percent }}%</div>
        </div>
        <div class="info">
            <h2 class="wow fadeInUp" data-wow-duration="2s">{{ $title }}</h2>
            <p class="wow fadeInUp" data-wow-duration="2.5s">{{ $about }}</p>
            <a href="{{ url('courses/'.$recent) }}" class="btn block-btn wow fadeInUp">@lang('courses.continue')</a>
        </div>
        <div class="clearfix"></div>
    </div>
    ...
    @foreach ($courses as $course)
        ...
        <div class="other-courses">
            <div class="decor">
                <img class="wow fadeIn" data-wow-duration="2.5s" src="{{ asset('img/icons/'.$state.'-plain-black.svg') }}" alt="">
            </div>
            <div class="info">
                <h2 class="wow fadeInUp" data-wow-duration="2s">{{ $course->title }}</h2>
                <div class="type wow fadeInRight" data-wow-duration="2s" style="padding-right: calc({{ $percent }}% - 30px)">
                    {{ $percent }}%
                </div>
                <div class="line">
                    <div class="progress wow slideInRight" data-wow-duration="2.5s" style="width: {{ $percent }}%"></div>
                </div>
                <a href="{{ url('courses/'.$state) }}" class="btn block-btn wow fadeInUp">@lang('courses.continue')</a>
            </div>
            <div class="clearfix"></div>
        </div>
        @endif
